...
impl SkiaScenePainter<'_> {
    ...
    fn set_paint_brush<'a>(
        &mut self,
        brush: impl Into<multirender::PaintRef<'a>>,
        brush_transform: Option<kurbo::Affine>,
    ) {
        let brush: multirender::PaintRef<'a> = brush.into();
        match brush {
            multirender::Paint::Solid(alpha_color) => {
                self.cache.paint.set_color4f(
                    sk_peniko::color4f_from_alpha_color(alpha_color),
                    &ColorSpace::new_srgb(),
                );
            }
            multirender::Paint::Gradient(gradient) => {
                self.cache
                    .paint
                    .set_shader(sk_peniko::shader_from_gradient(gradient, brush_transform));
            }
            multirender::Paint::Image(image_brush) => {
            ...
            multirender::Paint::Custom(_) => unreachable!(), // ToDo: figure out what to do with this
        }
    }
    ...
    fn get_or_cache_font(
        ...
        normalized_coords: &[multirender::NormalizedCoord],
        font_size: f32,
        hint: bool,
    ) -> Option<Font> {
...
impl PaintScene for SkiaScenePainter<'_> {
    ...
    fn draw_glyphs<'a, 's: 'a>(
        ...
        font_size: f32,
        hint: bool,
        normalized_coords: &'a [multirender::NormalizedCoord],
...
mod sk_peniko {
    ...
    use skia_safe::{AlphaType as SkAlphaType, shaders};
    ...
    pub(super) fn shader_from_gradient(
        ...
        match gradient.kind {
            GradientKind::Linear(linear_gradient_position) => {
                ...
                let gradient =
                    skia_safe::gradient_shader::Gradient::new(gradient_colors, interpolation);
            ...
            GradientKind::Sweep(sweep_gradient_position) => {
                ...
                let start_angle = rad_to_deg(sweep_gradient_position.start_angle);
                let end_angle = rad_to_deg(sweep_gradient_position.end_angle);
